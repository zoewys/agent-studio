...
function buildWorkflowComposerPlaceholder(
  run: UseWorkflowsResult['selectedRun'],
  selectedAgent: AgentDefinition | null,
  selectedStepState: { status?: string } | null,
  selectedExecution: { sessionId?: string } | null
): string {
  if (!run) return '请先启动一个工作流...'
  ...
  const status = selectedStepState?.status
  if (status === 'running') return '向运行中的 agent 发送消息...'
  if (status === 'error') return '输入修复指令，例如「请输出合法的 handoff JSON...」'
  if (status === 'done' || status === 'stale') return '继续此会话——下游步骤将被标记为过期...'
  if (status === 'awaiting-confirm') return '继续对话——handoff 将被重新生成...'
  return '当前步骤无法对话'
}
